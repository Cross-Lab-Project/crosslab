...
import { DeviceName, deviceData, deviceNames } from '../../data/devices/index.spec';
...
> {
        ...
        testSuite.addTest(
          new Mocha.Test(
            ...
            async function () {
              ...
              await data.repository.write(model, data.entityData[key].request);
              assert(data.validateWrite(model, data.entityData[key].request));
            },
          ),
        );
      }
  ...
  validateCreate(model: DeviceModel, data?: Device<'request'> | undefined): boolean {
    ...
    switch (model.type) {
      case 'cloud instantiable':
        ...
        return instantiableCloudDeviceRepositoryTestSuite.validateCreate(model, data);
      case 'device':
        assert(data?.type === model.type);
        return concreteDeviceRepositoryTestSuite.validateCreate(model, data);
      case 'edge instantiable':
        assert(data?.type === model.type);
        return instantiableBrowserDeviceRepositoryTestSuite.validateCreate(model, data);
      case 'group':
        assert(data?.type === model.type);
  ...
  validateWrite(model: DeviceModel, data: DeviceUpdate<'request'>): boolean {
    switch (model.type) {
      case 'cloud instantiable':
        assert(data?.type === model.type);
        return instantiableCloudDeviceRepositoryTestSuite.validateWrite(model, data);
      case 'device':
        assert(data?.type === model.type);
        return concreteDeviceRepositoryTestSuite.validateWrite(model, data);
      case 'edge instantiable':
        assert(data?.type === model.type);
        return instantiableBrowserDeviceRepositoryTestSuite.validateWrite(model, data);
      case 'group':
        assert(data?.type === model.type);
  ...
  validateFormat(
    ...
    switch (model.type) {
      case 'cloud instantiable':
        assert(data?.type === model.type);
        return instantiableCloudDeviceRepositoryTestSuite.validateFormat(model, data);
      case 'device':
        assert(data?.type === model.type);
        return concreteDeviceRepositoryTestSuite.validateFormat(model, data);
      case 'edge instantiable':
        assert(data?.type === model.type);
        return instantiableBrowserDeviceRepositoryTestSuite.validateFormat(model, data);
      case 'group':
        assert(data?.type === model.type);
  ...
  compareModels(
    ...
    switch (firstModel.type) {
      ...
      case 'device':
        assert(firstModel.type === secondModel.type);
        return concreteDeviceRepositoryTestSuite.compareModels(firstModel, secondModel);
      case 'edge instantiable':
        assert(firstModel.type === secondModel.type);
      ...
      case 'group':
        assert(firstModel.type === secondModel.type);
        return deviceGroupRepositoryTestSuite.compareModels(firstModel, secondModel);
    }
  }
  ...
  compareFormatted(first: Device<'response'>, second: Device<'response'>): boolean {
    switch (first.type) {
      case 'cloud instantiable':
        ...
        return instantiableCloudDeviceRepositoryTestSuite.compareFormatted(first, second);
      case 'device':
        if (!(first.type === second.type)) return false;
