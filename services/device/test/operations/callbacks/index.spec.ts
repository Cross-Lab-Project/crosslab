...
export default function (context: Mocha.Context, testData: TestData) {
  ...
  addTest(
    ...
    async function () {
      const res = await supertest(app).post("/callbacks/device").send({content: "test"});
  ...
  addTest(
    ...
    async function () {
      const res = await supertest(app).post("/callbacks/device").send({content: null});
  ...
  addTest(
    ...
    async function () {
      ...
      assert.strictEqual(res.status, 400);
      assert.strictEqual(res.body.error, MalformedBodyError.name);
      assert.strictEqual(res.body.message, "Callbacks require property 'callbackType'");
    },
  );
  ...
  addTest(
    ...
    async function () {
      const res = await supertest(app).post("/callbacks/device").send({callbackType: 1});
