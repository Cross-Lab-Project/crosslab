...
export default function (context: Mocha.Context, testData: TestData) {
  ...
  addTest(suite, "should return the flattened formatted device group", async function () {
    for (const deviceGroupName of deviceGroupNames) {
      ...
      assert(result.status === 200);
      assert(
        deviceRepositoryTestSuite.validateFormat(deviceGroup.model, result.body, {
          flatten: true,
        }),
      );
    }
  ...
  addTest(
    ...
    async function () {
      await assert.rejects(
        async () => {
          await getDevicesByDeviceId({device_id: "non-existent"}, testData.userData);
        },
        error => {
