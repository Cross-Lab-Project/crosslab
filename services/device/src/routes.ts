...
router.patch(
  ...
  async (
    ...
    try {
      ...
      const result = await operations.patchDevicesByDeviceId(parameters, body, userData);
  ...
  try {
    if (!validateGetDevicesByDeviceIdAvailabilityInput(parameters)) {
      throw new ValidationError(
        "Request validation failed",
        (
          validateGetDevicesByDeviceIdAvailabilityInput as unknown as {errors: object}
        ).errors,
        400,
      );
    }
    ...
    if (!validateGetDevicesByDeviceIdAvailabilityOutput(result)) {
      throw new ValidationError(
        "Response validation failed",
        (
          validateGetDevicesByDeviceIdAvailabilityOutput as unknown as {errors: object}
        ).errors,
        500,
      );
    }
  ...
  try {
    if (!validateDeleteDevicesByDeviceIdAvailabilityInput(parameters)) {
      throw new ValidationError(
        "Request validation failed",
        (
          validateDeleteDevicesByDeviceIdAvailabilityInput as unknown as {errors: object}
        ).errors,
        400,
      );
    }
    ...
    if (!validateDeleteDevicesByDeviceIdAvailabilityOutput(result)) {
      throw new ValidationError(
        "Response validation failed",
        (
          validateDeleteDevicesByDeviceIdAvailabilityOutput as unknown as {errors: object}
        ).errors,
        500,
      );
    }
  ...
  try {
    if (!validatePostDevicesByDeviceIdAvailabilityInput(parameters, body)) {
      throw new ValidationError(
        "Request validation failed",
        (
          validatePostDevicesByDeviceIdAvailabilityInput as unknown as {errors: object}
        ).errors,
        400,
      );
    }
    ...
    if (!validatePostDevicesByDeviceIdAvailabilityOutput(result)) {
      throw new ValidationError(
        "Response validation failed",
        (
          validatePostDevicesByDeviceIdAvailabilityOutput as unknown as {errors: object}
        ).errors,
        500,
      );
    }
  ...
  try {
    if (!validatePostDevicesByDeviceIdWebsocketInput(parameters)) {
      throw new ValidationError(
        "Request validation failed",
        (
          validatePostDevicesByDeviceIdWebsocketInput as unknown as {errors: object}
        ).errors,
        400,
      );
    }

    const result = await operations.postDevicesByDeviceIdWebsocket(parameters, userData);

    if (!validatePostDevicesByDeviceIdWebsocketOutput(result)) {
      throw new ValidationError(
        "Response validation failed",
        (
          validatePostDevicesByDeviceIdWebsocketOutput as unknown as {errors: object}
        ).errors,
        500,
      );
    }
    ...
    try {
      if (!validatePostDevicesByDeviceIdSignalingInput(parameters, body)) {
        throw new ValidationError(
          "Request validation failed",
          (
            validatePostDevicesByDeviceIdSignalingInput as unknown as {errors: object}
          ).errors,
          400,
        );
      }
      ...
      if (!validatePostDevicesByDeviceIdSignalingOutput(result)) {
        throw new ValidationError(
          "Response validation failed",
          (
            validatePostDevicesByDeviceIdSignalingOutput as unknown as {errors: object}
          ).errors,
          500,
        );
      }
  ...
  try {
    if (!validateGetPeerconnectionsByPeerconnectionIdInput(parameters)) {
      throw new ValidationError(
        "Request validation failed",
        (
          validateGetPeerconnectionsByPeerconnectionIdInput as unknown as {errors: object}
        ).errors,
        400,
      );
    }
    ...
    if (!validateGetPeerconnectionsByPeerconnectionIdOutput(result)) {
      throw new ValidationError(
        "Response validation failed",
        (
          validateGetPeerconnectionsByPeerconnectionIdOutput as unknown as {
            errors: object;
          }
        ).errors,
        500,
  ...
  try {
    if (!validateDeletePeerconnectionsByPeerconnectionIdInput(parameters)) {
      throw new ValidationError(
        "Request validation failed",
        (
          validateDeletePeerconnectionsByPeerconnectionIdInput as unknown as {
            errors: object;
          }
        ).errors,
        400,
    ...
    if (!validateDeletePeerconnectionsByPeerconnectionIdOutput(result)) {
      throw new ValidationError(
        "Response validation failed",
        (
          validateDeletePeerconnectionsByPeerconnectionIdOutput as unknown as {
            errors: object;
          }
        ).errors,
        500,
...
router.patch(
  ...
  async (
    ...
    try {
      if (
        !validatePatchPeerconnectionsByPeerconnectionIdDeviceStatusInput(parameters, body)
      ) {
        throw new ValidationError(
          "Request validation failed",
          (
            validatePatchPeerconnectionsByPeerconnectionIdDeviceStatusInput as unknown as {
              errors: object;
            }
          ).errors,
          400,
      ...
      const result = await operations.patchPeerconnectionsByPeerconnectionIdDeviceStatus(
        parameters,
        body,
        userData,
      );

      if (!validatePatchPeerconnectionsByPeerconnectionIdDeviceStatusOutput(result)) {
        throw new ValidationError(
          "Response validation failed",
          (
            validatePatchPeerconnectionsByPeerconnectionIdDeviceStatusOutput as unknown as {
              errors: object;
            }
          ).errors,
          500,
