...
export function testSuiteRemove<K extends string, R extends AbstractRepository<object, unknown, unknown, Record<string, object>>>(
  ...
  for (const key in repositoryTestData.entityData) {
    testSuite.addTest(
      new Mocha.Test(`should remove a specific existing model (${key})`, async function () {
