...
export function testSuiteSave<K extends string, R extends AbstractRepository<object, unknown, unknown, Record<string, object>>>(
  ...
  for (const key in repositoryTestData.entityData) {
    testSuite.addTest(
      new Mocha.Test(`should save a valid model (${key})`, async function () {
        ...
        const savedModel = (await repositoryTestData.repository.save(model)) as ModelType<R>;
