...
export default function (context: Mocha.Context, testData: TestData) {
  ...
  addTest(suite, 'should create a new template correctly', async function () {
    for (const templateName of templateNames) {
      ...
      assert(templateRepositoryTestSuite.validateWrite(templateModel, template.request));
      assert(templateRepositoryTestSuite.validateFormat(templateModel, result.body));
    }
  });
