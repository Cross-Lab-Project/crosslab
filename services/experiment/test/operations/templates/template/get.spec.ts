...
import { getTemplatesByTemplateId } from '../../../../src/operations/templates.js';
...
export default function (context: Mocha.Context, testData: TestData) {
  ...
  addTest(suite, 'should return the formatted template', async function () {
    for (const templateName of templateNames) {
      ...
      assert(templateRepositoryTestSuite.validateFormat(template.model, result.body));
      assert(
        templateRepositoryTestSuite.compareFormatted(template.response, result.body),
      );
    }
  });
