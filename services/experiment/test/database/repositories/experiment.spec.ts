...
import { DeviceModel, ExperimentModel, InstanceModel } from '../../../src/database/model';
import { ExperimentRepository } from '../../../src/database/repositories/experiment';
import { Experiment, ExperimentOverview } from '../../../src/generated/types';
import { experimentUrlFromId } from '../../../src/methods/url';
import { ExperimentName } from '../../data/experiments/index.spec';
...
> {
    ...
    validateWrite(model: ExperimentModel, data: Partial<Experiment<'request'>>): boolean {
        ...
        if (data.roles) {
            ...
            for (let i = 0; i < model.roles.length; i++) {
                assert(
                    roleRepositoryTestSuite.validateWrite(model.roles[i], data.roles[i]),
                );
            }
        }
    ...
    validateFormat(model: ExperimentModel, data: Experiment<'response'>): boolean {
        ...
        const modelInstantiatedDevices = (
            model.devices.filter(
                (device) => device.instance !== undefined,
            ) as (DeviceModel & { instance: InstanceModel })[]
        ).map((device) => device.instance);

        assert.strictEqual(
            data.instantiatedDevices.length,
            modelInstantiatedDevices.length,
        );
    ...
    compareModels(
        ...
        for (let i = 0; i < firstModel.roles.length; i++) {
            assert(
                roleRepositoryTestSuite.compareModels(
                    firstModel.roles[i],
                    secondModel.roles[i],
                ),
            );
    ...
    compareFormatted(
        ...
        for (let i = 0; i < first.devices.length; i++) {
            assert(
                deviceRepositoryTestSuite.compareFormatted(
                    first.devices[i],
                    second.devices[i],
                ),
            );
        ...
        for (let i = 0; i < first.roles.length; i++) {
            assert(
                roleRepositoryTestSuite.compareFormatted(first.roles[i], second.roles[i]),
            );
        }
