...
import { LdapAuthenticationError, LdapBindError, LdapError } from "../types/errors"
 ...
 */
async function createUserToken(user: UserModel, expiresIn: number = 3600000): Promise<TokenModel> {
 ...
 * @throws {LdapAuthenticationError} Thrown when using invalid credentials.
 * @throws {LdapBindError} Thrown when ldap bind fails.
 * @throws {LdapError} Thrown when ldap search does not return any entries.
...
export async function loginTui(username: string, password: string): Promise<TokenModel | undefined> {
    ...
    try {
        ...
        if (err.code === 49) {
            throw new LdapAuthenticationError("TUI ldap authentication failed because of invalid login credentials", 401)
        } else {
    ...
    if (searchResult.searchEntries.length === 0) {
        throw new LdapError(`No entries found for dn "cn=${username},ou=user,o=uni" during ldap search`, 401)
    }
