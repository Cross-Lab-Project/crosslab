...
class TokenRepositoryTestSuite extends AbstractRepositoryTestSuite<TokenModel> {
    ...
    public async initialize(): Promise<void> {
        ...
        this.addTestToSuite(
            "additional",
            (data) => new Mocha.Test("should add scope to token model only if it is not already present", async function () {
                const repository = data.repository as TokenRepository
                const tokenModel: TokenModel = await repository.save(data.entityData["valid token 1"].model)
    ...
    compareModels(firstModel: TokenModel, secondModel: TokenModel, complete?: boolean): boolean {
        if (!complete) {
