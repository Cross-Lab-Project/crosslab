...
type SuiteName =
    ...
    | 'format'
    | 'additional'
type CustomRecord<K extends string | number | symbol, T> = Record<K, T> & {
    [k: string]: T
}
...
function getRepositoryClass<M extends Model>(model: {
    new (): M
}): { new (): AbstractRepository<M> } {
    switch (model) {
        case ActiveKeyModel:
            return ActiveKeyRepository as any
        case KeyModel:
...
export abstract class AbstractRepositoryTestSuite<M extends Model> {
            ...
            create: (() => {
                ...
                testSuite.addTest(
                    new Mocha.Test(
                        ...
                        async function () {
                            for (const key of getEntityNames(model)) {
                                const unitializedRepository: AbstractRepository<M> =
                                    new (getRepositoryClass(model) as any)()
                                try {
                        ...
                        async function () {
                            for (const key of getEntityNames(model)) {
                                const unitializedRepository: AbstractRepository<M> =
                                    new (getRepositoryClass(model) as any)()
                                try {
                                    await unitializedRepository.remove(
                                        entityData[key].model
                                    )
                                    fail()
                                } catch (error) {
                        ...
                        async function () {
                            for (const key of getEntityNames(model)) {
                                const unitializedRepository: AbstractRepository<M> =
                                    new (getRepositoryClass(model) as any)()
                                try {
                                    await unitializedRepository.save(
                                        entityData[key].model
                                    )
                                    fail()
                                } catch (error) {
    ...
    public addTestToSuite(
        suiteName: SuiteName,
        test: (data: RepositoryTestData<M>) => Mocha.Test
    ) {
    ...
    public addSuiteToSuite(
        suiteName: SuiteName,
        suite: (data: RepositoryTestData<M>) => Mocha.Suite
    ) {
        if (!this.testSuites || !this.repositoryTestData)
            throw new Error('Test suite has not been initialized')
    ...
    abstract compareFormatted(
        first: ModelType<M, 'response'>,
        second: ModelType<M, 'response'>
    ): boolean
