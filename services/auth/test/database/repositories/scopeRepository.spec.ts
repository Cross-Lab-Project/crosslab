...
class ScopeRepositoryTestSuite extends AbstractRepositoryTestSuite<ScopeModel> {
    ...
    compareModels(firstModel: ScopeModel, secondModel: ScopeModel, complete: boolean): boolean {
        if (!complete) {
            return firstModel.name === secondModel.name
        }

        assert(firstModel.name === secondModel.name, `${firstModel.name} !== ${secondModel.name}`)
        return true
    }
