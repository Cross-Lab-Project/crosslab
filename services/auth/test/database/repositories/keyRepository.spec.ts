...
class KeyRepositoryTestSuite extends AbstractRepositoryTestSuite<KeyModel> {
    ...
    compareModels(firstModel: KeyModel, secondModel: KeyModel, _complete?: boolean): boolean {
        assert(firstModel.alg === secondModel.alg)
        assert(JSON.stringify(firstModel.private_key) === JSON.stringify(secondModel.private_key))
        assert(JSON.stringify(firstModel.public_key) === JSON.stringify(secondModel.public_key))
