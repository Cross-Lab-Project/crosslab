...
type RoleWithLinks<T extends "all" | "request" | "response" = "all"> = ReplaceWith<Role<T>, "scopes", ScopeName[]>
type RoleModelWithLinks = ReplaceWith<ReplaceWith<RoleModel, "scopes", ScopeName[]>, "users", UserName[]>
...
export function resolveRole(
    ...
    return {
        request: {
            ...
            scopes: roleDataWithLinks[roleName].request.scopes?.map((scopeName) => resolveScope(scopeName).request)
        },
        model: {
            ...roleDataWithLinks[roleName].model,
            scopes: roleDataWithLinks[roleName].model.scopes.map((scopeName) => resolveScope(scopeName).model),
            users: roleDataWithLinks[roleName].model.users.map((userName) => userData[userName]!.model as UserModel)
        },
        response: {
            ...roleDataWithLinks[roleName].response,
            scopes: roleDataWithLinks[roleName].response.scopes?.map((scopeName) => resolveScope(scopeName).response)
        }
    }
}
