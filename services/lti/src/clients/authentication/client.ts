...
export class Client {
  ...
  public async logout(
    ...
    url: string = '/logout',
  ): Promise<schemas.LogoutResponse> {
    // match path to url schema
    const m = new RegExp('^(' + escapeRegex(this.BASE_URL) + ')?/?()(logout)?$').exec(
      url,
    );
    if (!m) throw new Error('Invalid url');
  ...
  public async getUser(url: string): Promise<schemas.GetUserResponse> {
    // match path to url schema
    const m = new RegExp(
      '^(' + escapeRegex(this.BASE_URL) + ')?/?(users/[^?]*?)()?$',
    ).exec(url);
    if (!m) throw new Error('Invalid url');
    let valid_url = '/' + m[2] + '';
    if (valid_url.startsWith('//')) valid_url = valid_url.slice(1);
  ...
  ): Promise<schemas.UpdateUserResponse> {
    // match path to url schema
    const m = new RegExp(
      '^(' + escapeRegex(this.BASE_URL) + ')?/?(users/[^?]*?)()?$',
    ).exec(url);
  ...
  public async getIdentity(
    url: string = '/identity',
  ): Promise<schemas.GetIdentityResponse> {
    // match path to url schema
    const m = new RegExp('^(' + escapeRegex(this.BASE_URL) + ')?/?()(identity)?$').exec(
      url,
    );
    if (!m) throw new Error('Invalid url');
  ...
  public async updateIdentity(
    url: string = '/identity',
    body?: schemas.UpdateIdentityRequest,
  ): Promise<schemas.UpdateIdentityResponse> {
    // match path to url schema
    const m = new RegExp('^(' + escapeRegex(this.BASE_URL) + ')?/?()(identity)?$').exec(
      url,
    );
  ...
  public async createToken(
    ...
    url: string = '/token',
  ): Promise<schemas.CreateTokenResponse> {
    // match path to url schema
