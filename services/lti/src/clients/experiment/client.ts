...
export class Client {
  ...
  public async listExperiments(
    url: string = '/experiments',
  ): Promise<schemas.ListExperimentsResponse> {
    // match path to url schema
    const m = new RegExp(
      '^(' + escapeRegex(this.BASE_URL) + ')?/?()(experiments)?$',
    ).exec(url);
  ...
  public async createExperiment(
    ...
    url: string = '/experiments',
  ): Promise<schemas.CreateExperimentResponse> {
    // match path to url schema
    const m = new RegExp(
      '^(' + escapeRegex(this.BASE_URL) + ')?/?()(experiments)?$',
    ).exec(url);
  ...
  public async updateExperiment(
    url: string,
    body?: schemas.UpdateExperimentRequest,
    changedURL?: string,
  ): Promise<schemas.UpdateExperimentResponse> {
    // match path to url schema
    const m = new RegExp(
      '^(' + escapeRegex(this.BASE_URL) + ')?/?(experiments/[^?]*?)()?$',
    ).exec(url);
  ...
  ): Promise<schemas.ListTemplateResponse> {
    // match path to url schema
    const m = new RegExp('^(' + escapeRegex(this.BASE_URL) + ')?/?()(templates)?$').exec(
      url,
    );
    if (!m) throw new Error('Invalid url');
  ...
  public async createTemplate(
    ...
    url: string = '/templates',
  ): Promise<schemas.CreateTemplateResponse> {
    // match path to url schema
    const m = new RegExp('^(' + escapeRegex(this.BASE_URL) + ')?/?()(templates)?$').exec(
      url,
    );
  ...
  public async getTemplate(url: string): Promise<schemas.GetTemplateResponse> {
    // match path to url schema
    const m = new RegExp(
      '^(' + escapeRegex(this.BASE_URL) + ')?/?(templates/[^?]*?)()?$',
    ).exec(url);
    if (!m) throw new Error('Invalid url');
    let valid_url = '/' + m[2] + '';
    if (valid_url.startsWith('//')) valid_url = valid_url.slice(1);
  ...
  ): Promise<schemas.UpdateTemplateResponse> {
    // match path to url schema
    const m = new RegExp(
      '^(' + escapeRegex(this.BASE_URL) + ')?/?(templates/[^?]*?)()?$',
    ).exec(url);
