...
export class Client {
    ...
    public async getDeviceAvailability(url: string,
            options?: {
                ...
                if (urlSuffix && !url.endsWith(urlSuffix)) url = appendToUrl(url, urlSuffix)
                const [device_id,] = validateUrl(new URL(url).toString(), this.baseUrl, '/devices/{}/availability')
        console.log("trying to fetch url:", url);
    ...
    public async createWebsocketToken(url: string,
            options?: {
                ...
                if (urlSuffix && !url.endsWith(urlSuffix)) url = appendToUrl(url, urlSuffix)
                const [device_id,] = validateUrl(new URL(url).toString(), this.baseUrl, '/devices/{}/websocket')
        console.log("trying to fetch url:", url);
     ...
     */
    public async sendSignalingMessage(url: string,sigMessage: Types.CreatePeerconnectionMessage<"request"> | Types.ClosePeerconnectionMessage<"request"> | Types.SignalingMessage<"request"> | Types.ConfigurationMessage<"request"> | Types.ExperimentStatusChangedMessage<"request">,
            options?: {
                headers?: [string, string][],}): Promise<void> {
                const urlSuffix = '/devices/{}/signaling'.split('{}').at(-1) ?? ''
                if (urlSuffix && !url.endsWith(urlSuffix)) url = appendToUrl(url, urlSuffix)
                const [device_id,] = validateUrl(new URL(url).toString(), this.baseUrl, '/devices/{}/signaling')
        console.log("trying to fetch url:", url);
	...
	[k: string]: unknown
            ...
            options?: {
                ...
                if (urlSuffix && !url.endsWith(urlSuffix)) url = appendToUrl(url, urlSuffix)
                const [peerconnection_id,] = validateUrl(new URL(url).toString(), this.baseUrl, '/peerconnections/{}/device_status')
        console.log("trying to fetch url:", url);
