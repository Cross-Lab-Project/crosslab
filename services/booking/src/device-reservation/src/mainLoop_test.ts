...
mocha.describe("mainLoop.ts", function () {
    ...
    mocha.beforeEach(async function () {
        ...
        try {
            ...
            await db.execute("INSERT INTO reservation (`id`, `device`, `start`, `end`, `bookingreference`) VALUES (2,?,?,?,?)", ["http://localhost/device/superDevice", dayjs("2022-06-27T00:20:00Z").toDate(), dayjs("2022-06-27T00:30:00Z").toDate(), "http://localhost/unitTestPrefill/2"]);
            await db.execute("INSERT INTO reservation (`id`, `device`, `start`, `end`, `bookingreference`) VALUES (3,?,?,?,?)", ["http://localhost/device/superDevice", dayjs("2022-06-27T01:00:00Z").toDate(), dayjs("2022-06-27T02:00:00Z").toDate(), "http://localhost/unitTestPrefill/3"]);
            db.end();
        } catch (err) {
    ...
    mocha.it("mainLoop.ts get existing", async () => {
        ...
        if (data.Type != ReservationRequest.Get) {
            throw new Error("Reservation wrong type, should be Get, got " + data.ReservationRequest);
        }
    ...
    mocha.it("mainLoop.ts basic reservate -> get -> delete", async () => {
        ...
        if (data.Type != ReservationRequest.New) {
            throw new Error("Reservation wrong type, should be New, got " + data.ReservationRequest);
        }
        ...
        if (data.Type != ReservationRequest.Delete) {
            throw new Error("Reservation wrong type, should be Delete, got " + data.ReservationRequest);
        }
        ...
        if (data.Type != ReservationRequest.New) {
            throw new Error("Reservation wrong type, should be New (2), got " + data.ReservationRequest);
        }
        ...
        if (data.Type != ReservationRequest.Get) {
            throw new Error("Reservation wrong type, should be Get (2), got " + data.ReservationRequest);
        }
    });
