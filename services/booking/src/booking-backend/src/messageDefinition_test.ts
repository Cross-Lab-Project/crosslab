...
mocha.describe("messageDefinition.ts", function () {
    mocha.it("DeviceBookingRequest.fromString correct", async () => {
        let m = new DeviceBookingRequest(1n, new URL("http://localhost/device/superDevice"), 3, dayjs("2022-08-29T13:03:00Z"), dayjs("2022-08-29T13:04:00Z"));
        ...
        if(!dayjs("2022-08-29T13:03:00Z").isSame(data.Start)) {
            throw new Error("Mismatch in Start, should be 2022-08-29T13:03:00Z , is " + data.Start);
        }
