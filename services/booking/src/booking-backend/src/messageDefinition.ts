...
export class DeviceBookingRequest {
    ...
    static fromString(json: string): DeviceBookingRequest {
        ...
        return new DeviceBookingRequest(BigInt(data.BookingID), new URL(data.Device), data.Position, start, end);
    }

    public constructor(BookingID: bigint, Device: URL, Position: number, Start: dayjs.Dayjs, End: dayjs.Dayjs) {
