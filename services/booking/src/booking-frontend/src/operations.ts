...
import { Booking, Device, Experiment, Timeslot } from './generated/types';
...
export const postBooking: postBookingSignature = async (body, user) => {
  ...
  try {
    ...
    let [rows, fields]: [any, any] = await db.execute(
      ...
      [new Date(body.Time.Start), new Date(body.Time.End), body.Time, 'pending', user],
    );
...
export const deleteBookingByID: deleteBookingByIDSignature = async (parameters, user) => {
  ...
  try {
    let [rows, fields]: [any, any] = await db.execute(
      'SELECT `status`, `user` FROM booking WHERE `id`=? FOR UPDATE',
      [requestID],
    );
    if (rows.length === 0) {
      return {
...
export const deleteBookingByIDDestroy: deleteBookingByIDDestroySignature = async (
  ...
  if (
    user.JWT !== undefined &&
    user.JWT.scopes.includes('booking:destroy') &&
    BelongsToUs(new URL(user.JWT.url))
  ) {
  } else {
    return {
