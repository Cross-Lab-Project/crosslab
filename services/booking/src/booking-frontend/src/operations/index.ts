...
export const postBooking: postBookingSignature = async (request, body) => {
  ...
  try {
    ...
    let [rows, fields]: [any, any] = await db.execute(
      'INSERT INTO booking (`start`, `end`, `type`, `status`, `user`) VALUES (?,?,?,?,?)',
      [
        dayjs(body.Time.Start).toDate(),
        dayjs(body.Time.End).toDate(),
        body.Type,
        'pending',
    ...
    for (let i = 0; i < body.Devices.length; i++) {
      await db.execute(
        'INSERT INTO bookeddevices (`booking`, `originaldevice`, `originalposition`) VALUES (?,?,?)',
        [bookingID, body.Devices[i].ID, i],
      );
    }
    await db.commit();

    await request.authorization.relate(
...
export const getBookingByID: getBookingByIDSignature = async (request, parameters) => {
  ...
  try {
    ...
    let [rows, fields]: [any, any] = await db.execute(
      'SELECT `start`, `end`, `type`, `status`, `user`, `message` FROM booking WHERE id=?',
      [requestID],
    );
    if (rows.length == 0) {
      return {
    ...
    if (
      ...
      body.Booking.Status === 'active-pending' ||
      body.Booking.Status == 'active-rejected'
    ) {
      body.Locked = true;
    }
    ...
    [rows, fields] = await db.execute(
      'SELECT `originaldevice` FROM bookeddevices WHERE booking=? ORDER BY `originalposition` ASC',
      [requestID],
    );
...
export const deleteBookingByID: deleteBookingByIDSignature = async (
  request,
  parameters,
) => {
  let requestID: bigint = BigInt(parameters.ID);
...
export const patchBookingByID: patchBookingByIDSignature = async (
  request,
  parameters,
  body,
) => {
  ...
  try {
    let [rows, fields]: [any, any] = await db.execute(
      'SELECT `status`, `user`,`start`,`end` FROM booking WHERE `id`=? FOR UPDATE',
      [requestID],
    );
    if (rows.length === 0) {
      return {
    ...
    if (typeof body.Callback === 'string' && typeof body.Devices !== 'undefined' && (body.Devices as Device[]).length !== 0) {
      ...
      if (Devices.length !== 0) {
        ...
        let [deviceRows, deviceFields]: [any, any] = await db.execute(
          'SELECT MAX(`originalposition`) as max FROM bookeddevices WHERE `booking`=?',
          [requestID],
        );
        ...
        try {
          for (let i = 0; i < Devices.length; i++) {
            await db.execute(
              'INSERT INTO bookeddevices (`booking`, `originaldevice`, `originalposition`) VALUES (?,?,?)',
              [requestID, Devices[i].ID, i + toadd],
            );
          }
          await db.commit();
          db.beginTransaction();
          ...
          for (let i = 0; i < Devices.length; i++) {
            ...
            if (
              !channel.sendToQueue('device-booking', Buffer.from(s), { persistent: true })
            ) {
              throw new Error('amqp queue full');
            }
          }
      ...
      return {
        status: 500,
        body: "Unknown request type or wrong/missing parameter",
      };
    }
...
export const deleteBookingByIDDestroy: deleteBookingByIDDestroySignature = async (
  request,
  parameters,
) => {
  let requestID: bigint = BigInt(parameters.ID);
...
async function commonRemoveBooking(
  requestID: bigint,
): Promise<[404 | 200 | 423 | 500, string | null]> {
  let success: boolean = false;
  ...
  try {
    ...
    try {
      ...
      let [devicesRows, devicesFields]: [any, any] = await db.execute(
        'SELECT `id` FROM bookeddevices WHERE `booking`=? FOR UPDATE',
        [requestID],
      );
      for (let i = 0; i < devicesRows.length; i++) {
        if (
          !channel.sendToQueue(
            'device-freeing',
            Buffer.from(devicesRows[i].id.toString()),
            { persistent: true },
          )
        ) {
          throw new Error('amqp queue full');
    ...
    await db.execute('UPDATE booking SET `status`=?, `message`=? WHERE id=?', [
      'cancelled',
      'Cancelled by user',
      requestID,
    ]);
