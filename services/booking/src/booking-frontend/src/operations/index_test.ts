...
import {
  ...
  StartAMQPTestBooking,
  StopAMQPTestBooking,
  TestAMQPresultsBooking,
} from './indextest_helper_amqp_booking.js';
import {
...
mocha.describe('operations.ts', function () {
    ...
    try {
      ...
      try {
        await postBooking(req, {
          Time: {
            Start: dayjs('2000-01-01T07:00Z').toISOString(),
            End: dayjs('2000-01-01T08:00Z').toISOString(),
          },
          Devices: [
      ...
      if (Number(rows[0].n) != Number(before)) {
        throw new Error(
          'authorization failed but number of bookings was changed from ' +
          before +
          ' to ' +
          rows[0].n,
        );
      }
      if ((req as any).related.length != 0) {
        throw new Error('related not empty:' + (req as any).related.toString());
      }
      if (TestAMQPresultsBooking.size != 0) {
        throw new Error(
          'device reservation messages found ' + MapToString(TestAMQPresultsBooking),
        );
      }
    } finally {
      db.end();
  ...
  mocha.it('postBooking multiple devices', async function () {
    ...
    try {
      ...
      let result = await postBooking(req, {
        ...
        Devices: [
          { ID: 'http://localhost:10801/devices/10000000-0000-0000-0000-000000000000' },
          { ID: 'http://localhost:10801/devices/00000000-0000-0000-0000-000000000010' },
        ],
      });
      await sleep(250);
      ...
      if (
        !TestAMQPresultsBooking.has(
          bookingID.toString() +
          '-1-http://localhost:10801/devices/00000000-0000-0000-0000-000000000010',
        )
      ) {
        throw new Error(
          'wrong device reservation messages found' + MapToString(TestAMQPresultsBooking),
  ...
  mocha.it('getBookingByID success single (creator)', async function () {
    try {
      ...
      if (b.body.Booking.ID != 'http://localhost:10801/booking/1') {
  ...
  mocha.it('getBookingByID success multiple (creator)', async function () {
    try {
      ...
      if (b.body.Booking.ID != 'http://localhost:10801/booking/2') {
        throw new Error('bad id' + b.body.Booking.ID);
      }
  ...
  mocha.it('getBookingByID success group (creator)', async function () {
    try {
      ...
      if (b.body.Booking.ID != 'http://localhost:10801/booking/3') {
        throw new Error('bad id' + b.body.Booking.ID);
      }
  ...
  mocha.it('deleteBookingByID success single', async function () {
    ...
    try {
      ...
      if (TestAMQPresults.size != 1) {
        throw new Error(
          'wrong number of freed devices found ' + MapToString(TestAMQPresults),
        );
      }
  ...
  mocha.it('patchBookingByID authorization failed callback', async function () {
    ...
    try {
      ...
      try {
        await patchBookingByID(
          req,
          { ID: '1' },
          { Callback: 'http://localhost:10801/test_callbacks/callback-test-new' },
        );
      } catch (err) {
        if (err.message == 'test authorization failed') {
          isError = true;
      ...
      let [rows, _]: [any, any] = await db.execute(
        'SELECT count(*) AS n FROM bookingcallbacks WHERE `url`=?',
        ['http://localhost:10801/test_callbacks/callback-test-new'],
      );
      if (rows.length !== 1) {
        throw new Error('can not find booking');
      }
      ...
      [rows, _] = await db.execute('SELECT `status` FROM booking WHERE `id`=?', [
        BigInt(1),
      ]);
      if (rows.length !== 1) {
        throw new Error('can not find booking');
  ...
  mocha.it('patchBookingByID success unlocked callback', async function () {
    ...
    try {
      ...
      let res = await patchBookingByID(
        req,
        { ID: '1' },
        { Callback: 'http://localhost:10801/test_callbacks/callback-test-new' },
      );
      ...
      let [rows, _]: [any, any] = await db.execute(
        'SELECT `booking` FROM bookingcallbacks WHERE `url`=?',
        ['http://localhost:10801/test_callbacks/callback-test-new'],
      );
      if (rows.length == 0) {
        throw new Error('can not find booking');
  ...
  mocha.it('patchBookingByID empty device list does not change anything', async function () {
    let db = await mysql.createConnection(getSQLDNS());
    await db.connect();
    await StartAMQPTestBooking();
    try {
      ...
      let [rows, _]: [any, any] = await db.execute('SELECT `status` FROM booking WHERE `id`=?', [
        BigInt(1),
      ]);
      if (rows.length !== 1) {
        throw new Error('can not find booking');
  ...
  mocha.it('patchBookingByID success unlocked devices', async function () {
    ...
    try {
      ...
      let res = await patchBookingByID(
        ...
        {
          Devices: [
            { ID: 'http://localhost:10801/devices/20000000-0000-0000-0000-000000000000' },
          ],
        },
      );
      await sleep(250);
      ...
      if (TestAMQPresultsBooking.size != 1) {
        throw new Error(
          'wrong number devices were requested ' + MapToString(TestAMQPresultsBooking),
        );
      }

      if (
        !TestAMQPresultsBooking.has(
          '1-1-http://localhost:10801/devices/20000000-0000-0000-0000-000000000000',
        )
      ) {
        throw new Error(
          'wrong devices were requested ' + MapToString(TestAMQPresultsBooking),
        );
      }
      ...
      if (
        ...
        throw new Error(
          'wrong device number were requested ' + MapToString(TestAMQPresultsBooking),
        );
      }
  ...
  mocha.it('patchBookingByID success unlocked devices multiple', async function () {
    ...
    try {
      ...
      if (
        !TestAMQPresultsBooking.has(
          '1-2-http://localhost:10801/devices/00000000-0000-0000-0000-000000000010',
        )
      ) {
        throw new Error(
          'wrong devices were requested ' + MapToString(TestAMQPresultsBooking),
  ...
  mocha.it(
    'patchBookingByID wrong status unlocked devices (cancelled)',
    async function () {
      let db = await mysql.createConnection(getSQLDNS());
      await db.connect();
      await StartAMQPTestBooking();
      try {
        ...
        if (TestAMQPresultsBooking.size != 0) {
          throw new Error(
            'devices were requested ' + MapToString(TestAMQPresultsBooking),
          );
        }
  ...
  mocha.it(
    'patchBookingByID wrong status unlocked devices (rejected)',
    async function () {
      let db = await mysql.createConnection(getSQLDNS());
      await db.connect();
      await StartAMQPTestBooking();
  ...
  mocha.it(
    'patchBookingByID wrong status unlocked devices (active-pending)',
    async function () {
      let db = await mysql.createConnection(getSQLDNS());
      await db.connect();
      await StartAMQPTestBooking();
  ...
  mocha.it(
    'patchBookingByID wrong status unlocked devices (active-rejected)',
    async function () {
      let db = await mysql.createConnection(getSQLDNS());
      await db.connect();
      await StartAMQPTestBooking();
