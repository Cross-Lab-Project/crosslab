...
export async function startFakeServer() {
        ...
        switch (fakeServerConfig.device_service_status) {
            case 200:
                if (fakeServerConfig.device_wrong_device) {
                    res.send('{"url": "http://localhost:10801/devices/00000000-0000-0000-0000-111111111110", "name": "Test Group", "description": "Test group for unit tests", "type": "group", "owner": "http://localhost", "devices": [{"url": "http://localhost:10801/devices/10000000-0000-0000-0000-000000000000"}, {"url": "http://127.0.0.1:10802/devices/a0000000-0000-0000-0000-000000000000"}], "isPublic": true}');
                    return;
                }
                res.send('{"url": "http://localhost:10801/devices/00000000-0000-0000-0000-000000000001", "name": "Test Group", "description": "Test group for unit tests", "type": "group", "owner": "http://localhost", "devices": [{"url": "http://localhost:10801/devices/10000000-0000-0000-0000-000000000000"}, {"url": "http://127.0.0.1:10802/devices/a0000000-0000-0000-0000-000000000000"}], "isPublic": true}');
            case 404:
                res.status(404).send();
                return;
            case 500:
        ...
        switch (fakeServerConfig.device_service_status) {
            case 200:
                ...
                if (fakeServerConfig.device_not_available) {
                    res.send('{"url": "http://localhost:10801/devices/10000000-0000-0000-0000-000000000000", "name": "Test Device", "description": "Test device for unit tests", "type": "device", "owner": "http://localhost", "connected": true, "announcedAvailability": [], "isPublic": true}');
                    return;
                }
                res.send('{"url": "http://localhost:10801/devices/10000000-0000-0000-0000-000000000000", "name": "Test Device", "description": "Test device for unit tests", "type": "device", "owner": "http://localhost", "connected": true, "announcedAvailability": [{"start":"1999-01-01T00:00:00Z", "end": "1999-12-31T23:59:59Z"},{"start": "2022-06-20T00:00:00Z", "end": "2022-06-27T06:00:00Z"}, {"start": "2022-06-27T07:00:00Z", "end": "2022-07-01T23:59:59Z"}], "isPublic": true}');
                return;
            case 404:
                res.status(404).send();
                return;
    ...
    app.get('/devices/20000000-0000-0000-0000-000000000000', (req, res) => {
        switch (fakeServerConfig.device_service_status) {
            case 200:
                if (fakeServerConfig.device_single_is_group) {
                    res.send('{"url": "http://localhost:10801/devices/20000000-0000-0000-0000-000000000000", "name": "Test Group 3", "description": "Test group two for unit tests", "type": "group", "owner": "http://localhost", "devices": [{"url": "http://localhost:10801/devices/10000000-0000-0000-0000-000000000000"}, {"url": "http://127.0.0.1:10802/devices/a0000000-0000-0000-0000-000000000000"}], "isPublic": true}');
                    return;
                }
                if (fakeServerConfig.device_not_available) {
                    res.send('{"url": "http://localhost:10801/devices/20000000-0000-0000-0000-000000000000", "name": "Test Device 2", "description": "Test device for unit tests", "type": "device", "owner": "http://localhost", "connected": true, "announcedAvailability": [], "isPublic": true}');
                    return;
                }
                res.send('{"url": "http://localhost:10801/devices/20000000-0000-0000-0000-000000000000", "name": "Test Device 2", "description": "Test device for unit tests", "type": "device", "owner": "http://localhost", "connected": true, "announcedAvailability": [{"start":"1999-01-01T00:00:00Z", "end": "1999-12-31T23:59:59Z"},{"start": "2022-06-20T00:00:00Z", "end": "2022-06-27T06:00:00Z"}, {"start": "2022-06-27T07:00:00Z", "end": "2022-07-01T23:59:59Z"}], "isPublic": true}');
                return;
            case 404:
                res.status(404).send();
                return;
        ...
        switch (fakeServerConfig.schedule_service_status) {
            case 200:
                if (fakeServerConfig.schedule_service_booked) {
                    res.send('[{"Device": "http://localhost:10801/devices/10000000-0000-0000-0000-000000000000","Free":[],"Booked":[{"Start": "' + req.body.Time.Start + '","End": "' + req.body.Time.End + '"}]}]');
                } else {
                    res.send('[{"Device": "http://localhost:10801/devices/10000000-0000-0000-0000-000000000000","Free":[{"Start": "' + req.body.Time.Start + '","End": "' + req.body.Time.End + '"}],"Booked":[]}]');
                }
                return;
            case 400:
                res.status(400).send();
            ...
            switch (fakeServerConfig.proxy_server_status) {
                case 200:
                    if (fakeServerConfig.proxy_schedule_short_body) {
                        ...
                        res.send('[{"Device": "http://127.0.0.1:10802/devices/a0000000-0000-0000-0000-000000000000","Free":[],"Booked":[]}]');
                    } else {
    ...
    app.all('*', (req, res) => {
        ...
        res.status(999).send("Fake server got unknown request: " + req.path + " " + req.method);
    });
