...
for (const device2Type of deviceTypes) {
  for (const client1Type of clientTypes) {
    for (const client2Type of clientTypes) {
      describe(`${client1Type} Client to ${client2Type} Client GPIO tests without driver (device <-> ${device2Type})`, async function () {
        beforeEach(function () {
          this.timeout(120000);
          this.experiment = new ExperimentTest();
          this.experiment.addDevice(this, client1Type, 'device');
