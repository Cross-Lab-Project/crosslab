...
function shortDeviceList(devices: DeviceServiceTypes.DeviceOverview[], printNumbers: boolean=false) {
    let ret: string = "";
    for (const [idx, device] of devices.entries()) {
        ...
        ret += chalk.green(device.name) + " " + chalk.dim(device.url) + " " + chalk.dim("owned by " + device.owner) + "\n";
    }
    return ret;
}
...
export function device(program: Command, getClient: () => APIClient) {
    ...
    device.command('inspect').argument('[device url]').action(async (url?: string) => {
        const client = getClient();
