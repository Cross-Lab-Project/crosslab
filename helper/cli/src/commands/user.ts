...
export function user(program: Command, getClient: () => APIClient) {
    ...
    user.command('list').alias('ls').action(async () => {
        const client = getClient();

        console.log(await client.listUsers())
    })

    user.command('inspect').argument('[user url]').action(async (url?: string) => {
        const client = getClient();
    ...
    user.command('delete').argument('[user url]').action(async (url?: string) => {
        const client = getClient();
