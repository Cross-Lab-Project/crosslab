...
function parseResponses(
  ...
  for (const status in responses) {
    ...
    if (content && content['application/json'].schema) {
      extendedSchemas.push({
        ...
        'x-name': isService
          ...
          : formatName(operation.operationId ?? '', false) + 'Response' + status,
    ...
    if (response.headers)
      extendedSchemas.push(
        ...parseHeaders(response.headers, operation, path, method, status, isService),
      );
  }
    ...
    if (header.schema) {
      extendedSchemas.push({
        ...
        'x-name': isService
          ...
          : formatName(operation.operationId ?? '', false) + formatName(headerName),
