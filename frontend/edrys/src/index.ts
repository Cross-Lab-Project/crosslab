...
export class App extends LitElement {
  ...
  constructor() {
    ...
    Edrys.onReady(() => {
      ...
      if (Edrys.module) {
        this.login(Edrys.module.config.username, Edrys.module.config.password).then(
          success => success && this.startExperiment(),
        );
      }
    });
  }
  ...
  async experimentSetup(experiment: ExperimentServiceTypes.Experiment) {
    ...
    for (const device of experiment.devices ?? []) {
      if (device.device) {
        const setupProps = device.additionalProperties as {
          instanceUrl?: string;
          deviceToken?: string;
        };
        ...
        if (
          ...
          setupProps.instanceUrl &&
          setupProps.deviceToken &&
          deviceDetails.codeUrl
        ) {
          device.device = setupProps.instanceUrl;
          instances.push({
            codeUrl: deviceDetails.codeUrl,
            instanceUrl: setupProps.instanceUrl,
            deviceToken: setupProps.deviceToken,
          });
        }
      }
    }
    this.instances = instances;
    await this.client.updateExperiment(experiment.url!, {
      devices: experiment.devices,
    });
    while (true) {
      const instanceDetails = await Promise.all(
        instances.map(instance => this.client.getDevice(instance.instanceUrl)),
      );
          ...
          ${this.instances.map(
            instance => html`
              <iframe
                width="100%"
                height="800px"
                src="${instance.codeUrl}/?instanceUrl=${instance.instanceUrl}&deviceToken=${instance.deviceToken}"></iframe>
            `,
          )}
        </div>
      `;
