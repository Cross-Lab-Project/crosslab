...
async def test_create_device(aioresponses: aioresponses):
    ...
    request = json.loads(r'{"url":"https://KMlhNcfBAeYBDdlNWpqiS.uhisZnfxWisWyA346eVjpoeK4BZYjE6oK.ZDneOU1atITfp","type":"group","name":"deserunt est sint magna labore","isPublic":false,"bookingEndpoint":"http://f.yjzdF8TxgI0e.UVQRAyQNqrmskvLXxQSwdSjMZxmIrHvBPiS307XfTFjCY4..V.NUWazfNBmGFdcL","devices":[{"url":"https://iO.ftdxiSeFV-oumPBOjoCbwDypbzFf0kcgN3,iyJvCKIeH3KwDQHkah0G+BcBFq+fII"},{"url":"https://WOEvUdgAIpRBEVeeKJbJUFMYxRPsgpOCG.coy,e42gdrOXCLnYfGzKAMPo"}],"description":"in reprehenderit","owner":[{"url":"https://GNQkRLRSJXERGYXtciXG.yxgwcDUAiyUGjJY-aE5oeJy"}],"viewer":[{"url":"https://MXGuntpubSQBEWaFeD.rnlqrfnbIbi4uyKVsM2f8Y.I.N+8VVCJDodc+0qPPltOEcKiJfWd"},{"url":"http://zkxkKeJlGcaYu.udivLLttm6JR"},{"url":"http://aKzxRMFBfKa.gbSHJJql7Sy-HPro8lm3W"}]}')

    parameter_list = [{"changedUrl": "test_string", }, {}, ]

    for parameters in parameter_list:
        response_201_dict = json.loads(r'{"url":"https://KMlhNcfBAeYBDdlNWpqiS.uhisZnfxWisWyA346eVjpoeK4BZYjE6oK.ZDneOU1atITfp","type":"group","name":"deserunt est sint magna labore","isPublic":false,"bookingEndpoint":"http://f.yjzdF8TxgI0e.UVQRAyQNqrmskvLXxQSwdSjMZxmIrHvBPiS307XfTFjCY4..V.NUWazfNBmGFdcL","devices":[{"url":"https://iO.ftdxiSeFV-oumPBOjoCbwDypbzFf0kcgN3,iyJvCKIeH3KwDQHkah0G+BcBFq+fII"},{"url":"https://WOEvUdgAIpRBEVeeKJbJUFMYxRPsgpOCG.coy,e42gdrOXCLnYfGzKAMPo"}],"description":"in reprehenderit","owner":[{"url":"https://GNQkRLRSJXERGYXtciXG.yxgwcDUAiyUGjJY-aE5oeJy"}],"viewer":[{"url":"https://MXGuntpubSQBEWaFeD.rnlqrfnbIbi4uyKVsM2f8Y.I.N+8VVCJDodc+0qPPltOEcKiJfWd"},{"url":"http://zkxkKeJlGcaYu.udivLLttm6JR"},{"url":"http://aKzxRMFBfKa.gbSHJJql7Sy-HPro8lm3W"}]}')
        aioresponses.post(re.compile(re.escape(full_url)+r'(\?.*)?'), status=201, payload=response_201_dict)
        async with APIClient(BASE_URL) as client:
...
async def test_get_device(aioresponses: aioresponses):
    ...
    for parameters in parameter_list:
        response_200_dict = json.loads(r'{"url":"https://KMlhNcfBAeYBDdlNWpqiS.uhisZnfxWisWyA346eVjpoeK4BZYjE6oK.ZDneOU1atITfp","type":"group","name":"deserunt est sint magna labore","isPublic":false,"bookingEndpoint":"http://f.yjzdF8TxgI0e.UVQRAyQNqrmskvLXxQSwdSjMZxmIrHvBPiS307XfTFjCY4..V.NUWazfNBmGFdcL","devices":[{"url":"https://iO.ftdxiSeFV-oumPBOjoCbwDypbzFf0kcgN3,iyJvCKIeH3KwDQHkah0G+BcBFq+fII"},{"url":"https://WOEvUdgAIpRBEVeeKJbJUFMYxRPsgpOCG.coy,e42gdrOXCLnYfGzKAMPo"}],"description":"in reprehenderit","owner":[{"url":"https://GNQkRLRSJXERGYXtciXG.yxgwcDUAiyUGjJY-aE5oeJy"}],"viewer":[{"url":"https://MXGuntpubSQBEWaFeD.rnlqrfnbIbi4uyKVsM2f8Y.I.N+8VVCJDodc+0qPPltOEcKiJfWd"},{"url":"http://zkxkKeJlGcaYu.udivLLttm6JR"},{"url":"http://aKzxRMFBfKa.gbSHJJql7Sy-HPro8lm3W"}]}')
        aioresponses.get(re.compile(re.escape(full_url)+r'(\?.*)?'), status=200, payload=response_200_dict)
        async with APIClient(BASE_URL) as client:
