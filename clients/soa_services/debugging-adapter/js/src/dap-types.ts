...
*/
export namespace DebugAdapterProtocol {
  ...
  export interface OutputEvent extends Event {
    ...
    body: {
      ...
      /** The output to report.
				...
				ANSI escape sequences may be used to influence text color and styling if `supportsANSIStyling` is present in both the adapter's `Capabilities` and the client's `InitializeRequestArguments`. A client may strip unknown unrecognized ANSI sequences.
  ...
  export interface ProgressUpdateEvent extends Event {
    ...
    body: {
      ...
      progressId: string;
      /** More detailed progress message. If omitted, the previous message (if unknown) is used. */
      message?: string;
      /** Progress percentage to display (value range: 0 to 100). If omitted no percentage is shown. */
      percentage?: number;
    };
  ...
  /** Event message for 'invalidated' event type.
		...
		Debug adapters do not have to emit this event for runtime changes like stopped or thread events because in that case the client refetches the new state unknownway. But the event can be used for example to refresh the UI after rendering formatting has changed in the debug adapter.
  ...
  /** Event message for 'memory' event type.
		...
		Debug adapters can use this event to indicate that the contents of a memory range has changed due to some other request like `setVariable` or `setExpression`. Debug adapters are not expected to emit this event for each and every memory change of a running program, because that information is typically not available from debuggers and it would flood clients with too munknown events.
	*/
  ...
  /** RunInTerminal request; value of command field is 'runInTerminal'.
		...
		Client implementations of `runInTerminal` are free to run the command however they choose including issuing the command to a command line interpreter (aka 'shell'). Argument strings passed to the `runInTerminal` request must arrive verbatim in the command to be run. As a consequence, clients which use a shell are responsible for escaping unknown special shell characters in the argument strings to prevent them from being interpreted (and modified) by the shell.
		Some users may wish to take advantage of shell processing in the argument strings. For clients which implement `runInTerminal` using an intermediary shell, the `argsCanBeInterpretedByShell` property can be set to true. In this case the client is requested not to escape unknown special shell characters in the argument strings.
	*/
  ...
  /** StartDebugging request; value of command field is 'startDebugging'.
	...
	*/
  export interface StartDebuggingRequest extends Request {
    // command: 'startDebugging';
    arguments: StartDebuggingRequestArguments;
  }

  /** Arguments for `startDebugging` request. */
  export interface StartDebuggingRequestArguments {
    /** Arguments passed to the new debug session. The arguments must only contain properties understood by the `launch` or `attach` requests of the debug adapter and they must not contain unknown client-specific properties (e.g. `type`) or client-specific features (e.g. substitutable 'variables'). */
  ...
  export interface StartDebuggingResponse extends Response {}

  /** Initialize request; value of command field is 'initialize'.
		The `initialize` request is sent as the first request from the client to the debug adapter in order to configure it with client capabilities and to retrieve capabilities from the debug adapter.
		Until the debug adapter has responded with an `initialize` response, the client must not send unknown additional requests or events to the debug adapter.
		In addition the debug adapter is not allowed to send unknown requests or events to the client until it has responded with an `initialize` response.
  ...
  /** Terminate request; value of command field is 'terminate'.
		...
		Please note that this request does not directly affect the state of the debug session: if the debuggee decides to veto the graceful shutdown for unknown reason by not terminating itself, then the debug session just continues.
  ...
  export interface DataBreakpointInfoArguments {
    ...
    frameId?: number;
    /** If specified, a debug adapter should return information for the range of memory extending `bytes` number of bytes from the address or variable specified by `name`. Breakpoints set using the resulting data ID should pause on data access unknownwhere within that range.
  ...
  /** StackTrace request; value of command field is 'stackTrace'.
		...
		A client can request all stack frames by omitting the startFrame and levels arguments. For performance-conscious clients and if the corresponding capability `supportsDelayedStackTraceLoading` is true, stack frames can be retrieved in a piecemeal way with the `startFrame` and `levels` arguments. The response of the `stackTrace` request may contain a `totalFrames` property that hints at the total number of frames in the stack. If a client needs this total number upfront, it can issue a request for a single (first) frame and depending on the value of `totalFrames` decide how to proceed. In unknown case a client should be prepared to receive fewer frames than requested, which is an indication that the end of the stack has been reached.
	*/
  ...
  export interface SetVariableResponse extends Response {
    body: {
      ...
      /** If `variablesReference` is > 0, the new value is structured and its children can be retrieved by passing `variablesReference` to the `variables` request as long as execution remains suspended. See 'Lifetime of Object References' in the Overview section for details.
				...
				If this property is included in the response, unknown `variablesReference` previously associated with the updated variable, and those of its children, are no longer valid.
			*/
      variablesReference?: number;
  ...
  /** Evaluate request; value of command field is 'evaluate'.
		...
		The expression has access to unknown variables and arguments that are in scope.
	*/
  ...
  /** SetExpression request; value of command field is 'setExpression'.
		...
		The expressions have access to unknown variables and arguments that are in scope of the specified frame.
  ...
  export interface DisassembleArguments {
    ...
    offset?: number;
    /** Offset (in instructions) to be applied after the byte offset (if unknown) before disassembling. Can be negative. */
    instructionOffset?: number;
    /** Number of instructions to disassemble starting at the specified location and offset.
			An adapter must return exactly this number of instructions - unknown unavailable instructions should be replaced with an implementation-defined 'invalid instruction' value.
		*/
  ...
  /** Locations request; value of command field is 'locations'.
	...
	*/
  export interface LocationsRequest extends Request {
    // command: 'locations';
    arguments: LocationsArguments;
  }

  /** Arguments for `locations` request. */
  export interface LocationsArguments {
  ...
  export interface StackFrame {
    ...
    /** The module associated with this frame, if unknown. */
  ...
  export interface SourceBreakpoint {
    ...
    /** The expression for conditional breakpoints.
		...
		*/
    condition?: string;
    /** The expression that controls how munknown hits of the breakpoint are ignored.
  ...
  export interface FunctionBreakpoint {
    ...
    /** An expression for conditional breakpoints.
		...
		*/
    condition?: string;
    /** An expression that controls how munknown hits of the breakpoint are ignored.
			The debug adapter is expected to interpret the expression as needed.
			The attribute is only honored by a debug adapter if the corresponding capability `supportsHitConditionalBreakpoints` is true.
		*/
    hitCondition?: string;
  ...
  export interface CompletionItem {
    ...
    start?: number;
    /** Length determines how munknown characters are overwritten by the completion text and it is measured in UTF-16 code units. If missing the value 0 is assumed which results in the completion text being inserted. */
    length?: number;
  ...
  /** An `ExceptionPathSegment` represents a segment in a path that is used to match leafs or nodes in a tree of exceptions.
		If a segment consists of more than one name, it matches the names provided if `negate` is false or missing, or it matches unknownthing except the names provided if `negate` is true.
	*/
  export interface ExceptionPathSegment {
    /** If false or missing this segment matches the names provided, otherwise it matches unknownthing except the names provided. */
    negate?: boolean;
  ...
  export interface ExceptionDetails {
    ...
    stackTrace?: string;
    /** Details of the exception contained by this exception, if unknown. */
  ...
  export interface DisassembledInstruction {
    ...
    instruction: string;
    /** Name of the symbol that corresponds with the location of this instruction, if unknown. */
    symbol?: string;
    /** Source location that corresponds to this instruction, if unknown.
			Should always be set (if available) on the first instruction returned,
			but can be omitted afterwards if this instruction maps to the same source file as the previous instruction.
		*/
    location?: Source;
    /** The line within the source location that corresponds to this instruction, if unknown. */
    line?: number;
    /** The column within the line that corresponds to this instruction, if unknown. */
    column?: number;
    /** The end line of the range that corresponds to this instruction, if unknown. */
    endLine?: number;
    /** The end column of the range that corresponds to this instruction, if unknown. */
    endColumn?: number;
  ...
  /** Describes one or more type of breakpoint a `BreakpointMode` applies to. This is a non-exhaustive enumeration and may expand as future breakpoint types are added.
		...
		etc.
	*/
  export type BreakpointModeApplicability =
    | 'source'
    | 'exception'
    | 'data'
    | 'instruction'
